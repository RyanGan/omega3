...
# code needs to account for missingness by specific variables (i.e. income) 
# I have an na.omit function built in to the summary
# calculate n
tab1_n_df <- baseline_df %>% 
  ...
  summarise(n = n(), mean_age = n(), age50_n = sum(age_bi),
    ...
    income_gt_40k_n = sum(na.omit(inc)), pack_years_gt_10_n = sum(py10),
...
tab1_perc_df <- baseline_df %>% 
  ...
  summarise(n = n(), mean_age = mean(age), age50_perc = sum(age_bi)/n,
    ...
    income_gt_40k = sum(na.omit(inc))/length(na.omit(inc)),  # correctly calculates
    pack_years_gt_10 = sum(py10)/n,
...
tab3_perc_df <- timevarying_df %>% 
  ...
  summarise(n = n(), mean_age = mean(age), age50_perc = sum(age_bi)/n,
    female_perc = sum(sex)/n, nhw_perc = sum(race_bi)/n, ed_gt_hs = sum(educ)/n,
    income_gt_40k = sum(na.omit(inc))/length(na.omit(inc)), # handles missingness
    pack_years_gt_10 = sum(py10)/n, shared_epitope_pos = sum(se_bi)/n, 
    n3_supp_use = sum(omega3_bi)/n, multi_vit_use = sum(multivits_bi)/n, 
    rf_pos = sum(rfneph)/n, crp_pos = sum(crp)/n, ccp2_pos = sum(ccp2)/n) %>% 
  round(., 2) # round to two digits
...
# fill test of mean difference in age p value
table3_matrix[2, 6] <- as.numeric(age_p)
...
for(i in 1:length(var_loop)){
  ...
  fisher_p <- tidy(fisher.test(tab)) %>% select(p.value)
  # fill in matrix
  table3_matrix[i + 2, 6] <- round(as.numeric(fisher_p), 3)
...
knitr::kable(table3_df, caption = paste0("Table 1: Demographic and descriptive",
  " characteristics of ACPA positive subjects by Inflammatory Arthritis", 
  " (IA) status at baseline"))
...
I'll write some stuff in later....